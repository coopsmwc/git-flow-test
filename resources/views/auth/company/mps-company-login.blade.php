                ...
                <div class="login-container">
                    <div class="login-wrapper">
                        ...
                        <div class="form-wrap">
                            ...
                            <h3>{{ __('Login') }}</h3>
                            <form role="form" action="{{ route('company-admin-login', $stub) }}" method="POST" id="login-form" autocomplete="off">
                                @csrf
                                <div class="form-group">
                                    <label for="email" class="sr-only">{{ __('E-Mail Address') }}</label>
                                    <input id="email" type="email" class="form-control{{ $errors->has('email') || $errors->has('id') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" required autofocus>
